 ...
 * is that the app keeps working on the hardware a lot of Philippine shoppers
 * actually carry.
 *
 ...
 * shopper would get no address bar to check. Custom Tabs is the real browser,
 * with the real lock icon and the session the shopper may already have.
 *
...
class GoogleAuthFlow @Inject constructor() {
     ...
     * and a sign-in silently dropped for that is a shopper pressing the button
     ...
     * the account screen offers guest browsing and nothing else, and checkout
     * never asked for an account in the first place.
     */
     ...
     * their own choosing and sign the shopper into somebody else's account.
     */
 ...
 * carrying an authorization code of its own and sign the shopper into an
...
internal fun readRedirect(
    ...
    if (error != null) {
        // `access_denied` is the shopper pressing cancel or closing the tab.
...
/** What came back from the browser. Not yet a session — see AccountRepository. */
sealed interface GoogleAuthResult {
    ...
    /** The shopper backed out. Say nothing; just stop the spinner. */
