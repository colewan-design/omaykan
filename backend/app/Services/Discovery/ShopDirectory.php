...
/**
 * The shops a customer can order from, and what each one looks like on a card.
 *
 * Lived in StoreDirectoryController until the town and category landing pages
 * needed the same list. Both read it from here, because "which shops are
 * listed" is one rule: a page that computed its own could show a shop the
 * directory hides, and that card would lead to a closed storefront.
 */
class ShopDirectory
{
    ...
    /**
     * Each listed shop with its card, sorted as the directory shows them.
     *
     * The Store travels with its row because the landing pages classify on
     * fields the public card leaves out (the organization id, the free-text
     * address as typed).
     *
     * @return Collection<int, array{store: Store, row: array<string, mixed>}>
     */
    public function listings(string $term = '', ?float $lat = null, ?float $lng = null): Collection
    {
        ...
        return $this->sort($listings, $lat !== null);
    }
    ...
    private function query(string $term)
    {
        return Store::query()
            ...
            // dates and a config flag, and is applied to the results in listings.
     ...
     */
    public static function kmBetween(float $fromLat, float $fromLng, float $toLat, float $toLng): float
    {
        $fromLatRad = deg2rad($fromLat);
        $toLatRad = deg2rad($toLat);
        $deltaLat = $toLatRad - $fromLatRad;
        $deltaLng = deg2rad($toLng - $fromLng);

        $a = sin($deltaLat / 2) ** 2 + cos($fromLatRad) * cos($toLatRad) * sin($deltaLng / 2) ** 2;
    ...
    private function distanceKm(Store $store, ?float $lat, ?float $lng): ?float
    {
        ...
        return round(self::kmBetween((float) $store->lat, (float) $store->lng, $lat, $lng), 2);
    }

    /**
     * Open shops first, then by distance when there is a location, then by
     * name. A paused shop stays in the list (see listings) but is not what a
     * hungry person should see first.
     *
     * Shops with no pin, or too far to be a delivery, keep their alphabetical
     * order behind the ones that can be measured — they are still listed,
     * because a shop without coordinates is not a shop that is closed.
     *
     * @param  Collection<int, array{store: Store, row: array<string, mixed>}>  $listings
     * @return Collection<int, array{store: Store, row: array<string, mixed>}>
     */
    private function sort(Collection $listings, bool $hasLocation): Collection
    {
        if (! $hasLocation) {
            // Stable: the query already ordered by name, and that order holds
            // within the open and the paused halves.
            return $listings->sortBy(fn (array $listing): int => $listing['row']['orderingPaused'] ? 1 : 0)->values();
        }

        return $listings
            ->sortBy(function (array $listing): array {
                $row = $listing['row'];
